...
#include <stdint.h>
#define MAX_TIME 85
#define DHT11PIN 7				   //pin number 

int dht11_val[5]={0,0,0,0,0};

int dht11_tryread_val(int key){
...
int dht11_read_val(int key){
    int attempts=ATTEMPTS;
    if(wiringPiSetup()==-1)
        exit(1);
    while(attempts){                        //you have 5 times to retry
        int success = dht11_tryread_val(key);     //get result including printing out
